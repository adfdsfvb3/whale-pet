...
- **设置面板**：API key（密文）、模型选择（`deepseek-v4-pro` / `deepseek-v4-flash` 走 dsh agent，`deepseek-chat` / `deepseek-reasoner` 纯聊天）、朗读开关与语速、宠物大小（小/中/大）、待机小动作频率、开机自动启动——全部保存即生效，持久化在 `~/.whalepet.conf`
